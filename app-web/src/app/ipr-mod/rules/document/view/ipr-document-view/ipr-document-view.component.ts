...
@Component({
  selector: 'app-ipr-document-view',
  templateUrl: './ipr-document-view.component.html',
  styleUrls: ['./ipr-document-view.component.css']
})

export class IprDocumentViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.publishDate = formatDate(new Date());
    this.getLovData();
  ...
  onBeforeRuleSave() {
    ...
    if (!formValid) {
      this._matSnackbar.open("Please fill all values.", "Dismiss", {
        duration: 2000,
      });
      return false;
    }
    ...
    this._rulesService.saveRules(url, postObj).subscribe((data) => {
      ...
      if (data.status === ServiceResponse.STATUS_SUCCESS) {
        ...
        swal.fire({
          ...
          if (this.is_admin)
            this._router.navigate(['/iprdocuments']);
        });
      } else {
  ...
  saveRules() {
    ...
    this._rulesService.saveRules(_url,reqData).subscribe(res => {
      ...
      if (res.status === ServiceResponse.STATUS_SUCCESS) {
        ...
        swal.fire({
          text: 'Document rules saved successsfully',
          icon: 'success'
        }).then(() => {
